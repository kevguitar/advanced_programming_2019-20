...
- A `std::unique_ptr<T> p` has the following functions
   ...
   - `void reset(T* ptr)` Delete `current_ptr` (if any) and set `current_ptr = ptr`. If set to `nullptr`, then current_ptr is set to `nullptr`.
...
-  For a class `Foo`
    ...
    class Foo{
      ...
      Foo(Foo&& f); // move ctor; uses argvalue reference
